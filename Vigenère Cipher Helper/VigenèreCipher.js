/*
  ...
  It is named after a later French cryptographer Blaise de Vigenère, who had developed a stronger autokey cipher (a cipher that incorporates
  the message of the text into the key).

  The cipher is easy to understand and implement, but survived three centuries of attempts to break it, earning it the nickname "le chiffre
  ...
  characters only if they are part of the alphabet -- this is not the case here.
...
function convertChar(abc, char, key, mul) {
    return abc[(abc.length + abc.indexOf(char) + mul * abc.indexOf(key)) % abc.length];
}

function handleInput(abc, allKey, input, mul) {
    return input.split("").reduce((ret, char, idx) => {
        return ret += (abc.indexOf(char) < 0) ? char :
            convertChar(abc, char, allKey[idx % allKey.length], mul);
    }, "");
}
...
class VigenèreCipher {
    ...
    encode(input) {
        return handleInput(this.abc, this.key, input, 1);
    }
    decode(input) {
        return handleInput(this.abc, this.key, input, -1);
    }
